...
let collectdata = async () => {
    ...
    let Necklaces = data.filter(function (el) {
        return el.category == "Necklaces"
    })


append(Necklaces)
array.push(Necklaces)



}
// productno.innerText=array[0].length
collectdata()

let append= (Necklaces) => {
  if (!Necklaces) return;

  let products = document.getElementById("product-list");
  products.innerHTML = "";

  Necklaces.forEach((el) => {
